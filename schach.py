...
class Springer(Figur):
    ...
    def moegliche_zuege(self, brett: 'Schachbrett', beachte_bedrohung=True) -> Zuege:
        ...
        ret_schlaegt =[]
        ret_schlaegt.append(self.position.plus(1,2))
        ret_schlaegt.append(self.position.plus(2,1))
        ret_schlaegt.append(self.position.plus(-1,2))
        ret_schlaegt.append(self.position.plus(2,-1))
        ret_schlaegt.append(self.position.plus(-2,1))
        ret_schlaegt.append(self.position.plus(-1,-2))
        ...
        ret_schlaegt = [x for x in ret_schlaegt if x in brett.brett]
        bewegt = [x for x in ret_schlaegt]
        if beachte_bedrohung:
            ret_schlaegt2 = []
            ret_bewegt = []
            for zug in ret_schlaegt:
                self.probier_zug_aus_ob_er_legal_waere(brett, ret_schlaegt2, zug)
            for zug in bewegt:
                self.probier_zug_aus_ob_er_legal_waere(brett, ret_bewegt, zug)

            return Zuege(bewegt=ret_bewegt, schlaegt=ret_schlaegt2)
        return Zuege(bewegt=bewegt, schlaegt=ret_schlaegt)
